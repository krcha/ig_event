...
type ReviewedStructuredCorrectionVersionArgs = {
...
type ReviewedStructuredReceipt = Doc<"instagramSourceOccurrenceReceipts"> & {
  expectedOccurrences: SourceOccurrencePlan["expectedOccurrences"];
};

async function loadReviewedStructuredCorrectionContext(
  ctx: QueryCtx | MutationCtx,
  event: Doc<"events">,
  ...
  try {
    const parsedFields = JSON.parse(event.normalizedFieldsJson ?? "null") as unknown;
    ...
    throw new Error("Reviewed structured correction requires valid v2 evidence JSON.");
  }
  if (
    currentFields.extractionContractVersion !== "event_evidence_v2" ||
    currentFields.extractionIsEvent !== true ||
  ...
  if (
    ...
    sourceLink.sourceFingerprint !== currentFields.sourceOccurrenceSourceFingerprint ||
    ...
    throw new Error("Reviewed structured correction source link changed or is inconsistent.");
  }
  ...
  if (
    ...
    throw new Error("Reviewed structured correction receipt changed or is missing.");
  }
  const expectedKeys = receipt.expectedOccurrences.map((item) => item.key);
  const satisfiedOccurrenceKeys = receipt.satisfiedOccurrences.map((item) => item.key);
  ...
  if (
    ...
    receipt.expectedKeys.filter((key) => key === sourceLink.sourceOccurrenceKey).length !== 1 ||
    receipt.satisfiedKeys.filter((key) => key === sourceLink.sourceOccurrenceKey).length !== 1 ||
    matchingSatisfiedOccurrences.length !== 1 ||
    matchingSatisfiedOccurrences[0].eventId !== event._id ||
    receipt.satisfiedOccurrences.filter((item) => item.eventId === event._id).length !== 1
  ) {
    throw new Error("Reviewed structured correction receipt is ambiguous or incomplete.");
  }
  ...
  for (const satisfied of receipt.satisfiedOccurrences) {
    ...
    const representative =
      satisfied.eventId === event._id ? event : await ctx.db.get(satisfied.eventId);
    if (
      ...
      throw new Error("Reviewed structured correction receipt has a stale occurrence representative.");
    }
  }
...
/**
 * Returns the exact source/receipt revisions needed for a reviewed correction.
 * The write mutation below rechecks every value in one serializable transaction.
 */
export const getReviewedStructuredEvidenceCorrectionContext = query({
  ...
  handler: async (ctx, args) => {
    const authorization = await requireAdminOrServiceSecret(ctx, args.serviceSecret);
    if (authorization.kind !== "service") {
      throw new Error("Reviewed structured correction context requires service authentication.");
    }
    const event = await ctx.db.get(args.id);
    if (!event) throw new Error("Event not found.");
    if (event.status !== "approved" || isCrossPostCampaignLineageEvent(event)) {
      throw new Error("Reviewed structured correction context requires an eligible approved event.");
    }
    if (!(await isCanonicallyGroundedApprovedEvent(ctx, event))) {
      throw new Error("Reviewed structured correction context is not publicly source-grounded.");
    }
    const sourceLinks = await ctx.db
      .query("instagramEventSources")
      .withIndex("by_event", (q) => q.eq("eventId", event._id))
    ...
    if (sourceLinks.length !== 1) {
      throw new Error("Reviewed structured correction requires one exact source link.");
    }
    ...
    const receipt = await ctx.db
      ...
      .unique();
    if (!receipt) {
      throw new Error("Reviewed structured correction source receipt is missing.");
    }
...
/**
 * Applies a human-reviewed poster correction without rewriting the immutable
 * GPT extraction. Public fields are rebound in normalized evidence, the exact
 * occurrence receipt is kept coherent, and public grounding continues through
 * the explicit structured-human-review policy.
 */
export const repairReviewedStructuredEventEvidence = mutation({
  ...
  handler: async (ctx, args) => {
    const { actor, kind } = await requireAdminOrServiceSecret(ctx, args.serviceSecret);
    if (kind !== "service") {
      throw new Error("Reviewed structured correction requires service authentication.");
    }
    const moderationNote = args.moderationNote.normalize("NFKC").replace(/\s+/gu, " ").trim();
    if (moderationNote.length < 20) {
      throw new Error("Reviewed structured correction requires a substantive audit note.");
    }
    for (const value of [
      ...
      if (!Number.isSafeInteger(value)) {
        throw new Error("Reviewed structured correction requires safe optimistic revisions.");
      }
    }
    ...
    if (!event) throw new Error("Event not found.");
    if (event.status !== "approved") {
      throw new Error("Reviewed structured correction only accepts an approved event.");
    }
    if (isCrossPostCampaignLineageEvent(event)) {
      throw new Error("Campaign lineage events require their dedicated re-attestation operation.");
    }
    assertExpectedEventUpdatedAt(event.updatedAt, args.expectedUpdatedAt);
    if (event.normalizedFieldsJson !== args.expectedNormalizedFieldsJson) {
      throw new Error("Normalized event evidence changed before reviewed correction.");
    }
    if (!(await isCanonicallyGroundedApprovedEvent(ctx, event))) {
    ...
    if (
      ...
      throw new Error("Reviewed structured correction public fields are not canonical.");
    }
    ...
    if (
      ...
      throw new Error("Reviewed structured correction requires exact poster evidence.");
    }
    ...
    if (args.targetVenueId) {
      ...
      if (
        ...
        throw new Error("Reviewed structured correction target venue is not exact and public.");
      }
      ...
      if (venueFields.venueId !== targetVenue._id) {
        throw new Error("Reviewed structured correction target venue did not resolve exactly.");
      }
    } else {
      if (
        args.expectedTargetVenueUpdatedAt !== undefined ||
        ...
        throw new Error("Noncanonical venue correction cannot carry target venue revisions.");
      }
      venueFields = await resolveVenueDenormalizedFields(ctx, nextVenue);
      if (venueFields.venueId !== undefined) {
        throw new Error("A known public venue correction must bind its exact venueId.");
      }
    }
    ...
    const nextSignals = [
      ...new Set(
        [...currentSignals.filter((signal) => signal !== "time_tbd"), "requires_human_approval"],
      ),
    ];
    ...
    if (!isSensibleEventTitleForApproval(effectiveEvent)) {
      throw new Error("Reviewed structured correction title is not suitable for approval.");
    }
    if (!hasHumanReviewableStructuredSourceAttestation(
      nextNormalizedFieldsJson,
      effectiveEvent,
    )) {
      throw new Error("Reviewed structured correction did not bind all public fields.");
    }
    await assertPersistedServiceSourcePolicy(ctx, effectiveEvent, {
    ...
    if (!(await isCanonicallyGroundedApprovedEvent(ctx, effectiveEvent))) {
      throw new Error("Reviewed structured correction would not remain publicly grounded.");
    }
    ...
    for (const satisfied of receipt.satisfiedOccurrences) {
      ...
      if (
        ...
        throw new Error("Reviewed structured correction would invalidate a receipt occurrence.");
      }
    }
    ...
    await writeEventAuditLog(ctx, event._id, "reviewed_structured_evidence_corrected", {
      actor,
      note: moderationNote,
      patch: {
        policyVersion: 1,
...
/**
 * Corrects only the venue on an approved event with structured v2 evidence.
 * All other public fields and their provenance remain byte-for-byte unchanged,
 * while the exact source-occurrence receipt is updated in the same transaction.
 */
export const repairReviewedStructuredEventVenue = mutation({
  ...
  handler: async (ctx, args) => {
    const { actor, kind } = await requireAdminOrServiceSecret(ctx, args.serviceSecret);
    if (kind !== "service") {
      throw new Error("Reviewed structured venue correction requires service authentication.");
    }
    const moderationNote = args.moderationNote.normalize("NFKC").replace(/\s+/gu, " ").trim();
    const nextVenue = args.nextVenue.normalize("NFKC").replace(/\s+/gu, " ").trim();
    ...
    if (moderationNote.length < 20 || !nextVenue || !venueEvidence) {
      throw new Error("Reviewed structured venue correction requires exact evidence and an audit note.");
    }
    for (const value of [
      args.expectedUpdatedAt,
      args.expectedSourceLinkUpdatedAt,
      ...
      if (!Number.isSafeInteger(value)) {
        throw new Error("Reviewed structured venue correction requires safe optimistic revisions.");
      }
    }
    ...
    if (!event) throw new Error("Event not found.");
    if (event.status !== "approved") {
      throw new Error("Reviewed structured venue correction only accepts an approved event.");
    }
    if (isCrossPostCampaignLineageEvent(event)) {
      throw new Error("Campaign lineage events require their dedicated re-attestation operation.");
    }
    ...
    if (args.targetVenueId) {
      ...
      if (
        ...
        throw new Error("Reviewed structured venue target is not exact and public.");
      }
      venueFields = resolveVenueDenormalizedFieldsFromPublicVenues(
        [targetVenue],
        nextVenue,
      ...
      if (venueFields.venueId !== targetVenue._id) {
        throw new Error("Reviewed structured venue target did not resolve exactly.");
      }
    } else {
      if (
        args.expectedTargetVenueUpdatedAt !== undefined ||
    ...
    if (!hasHumanReviewableStructuredSourceAttestation(
      ...
      throw new Error("Reviewed structured venue correction did not bind all public fields.");
    }
    await assertPersistedServiceSourcePolicy(ctx, effectiveEvent, {
      allowHumanReviewedStructured: true,
    });
    ...
    for (const satisfied of receipt.satisfiedOccurrences) {
      ...
      if (
        ...
        throw new Error("Reviewed structured venue correction would invalidate a receipt occurrence.");
      }
    }
    ...
    await writeEventAuditLog(ctx, event._id, "reviewed_structured_venue_corrected", {
      actor,
      note: moderationNote,
      patch: {
        policyVersion: 1,
...
/**
 * Folds one reviewed weak teaser post into a stronger event post when the
 * physical venue is explicit but has no trustworthy canonical venue record.
 * Both post-specific receipts stay complete and auditable; the teaser row is
 * rejected in place instead of being deleted.
 */
export const foldReviewedStructuredPromotionVariant = mutation({
  ...
  handler: async (ctx, args) => {
    const authorization = await requireAdminOrServiceSecret(ctx, args.serviceSecret);
    if (authorization.kind !== "service") {
      throw new Error("Reviewed promotion folding requires service authentication.");
    }
    const operationId = args.operationId.trim();
    const moderationNote = args.moderationNote.normalize("NFKC").replace(/\s+/gu, " ").trim();
    ...
    if (
      ...
      new Set(campaignAnchors.map(normalizeLookup)).size !== campaignAnchors.length ||
    ...
    for (const value of [
      ...
      if (!Number.isSafeInteger(value)) {
        throw new Error("Reviewed promotion folding requires safe optimistic revisions.");
      }
    }
    ...
    if (
      ...
      throw new Error("Reviewed promotion folding requires two grounded approved events.");
    }

    const primaryContext = await loadReviewedStructuredCorrectionContext(ctx, primary, {
      expectedSourceLinkId: args.expectedPrimarySourceLinkId,
      expectedSourceLinkUpdatedAt: args.expectedPrimarySourceLinkUpdatedAt,
      expectedReceiptId: args.expectedPrimaryReceiptId,
      expectedReceiptUpdatedAt: args.expectedPrimaryReceiptUpdatedAt,
    });
    const variantContext = await loadReviewedStructuredCorrectionContext(ctx, variant, {
    ...
    if (
      ...
      normalizeInstagramPostUrl(primary.instagramPostUrl) ===
        normalizeInstagramPostUrl(variant.instagramPostUrl) ||
    ...
    if (args.targetVenueId) {
      ...
      if (
        ...
        throw new Error("Reviewed promotion fold target venue is not exact and public.");
      }
      venueFields = resolveVenueDenormalizedFieldsFromPublicVenues(
        [targetVenue],
        nextVenue,
      ...
      if (venueFields.venueId !== targetVenue._id) {
        throw new Error("Reviewed promotion fold target venue did not resolve exactly.");
      }
    } else {
      if (
        args.expectedTargetVenueUpdatedAt !== undefined ||
        ...
        throw new Error("Noncanonical promotion fold cannot carry target venue revisions.");
      }
      venueFields = await resolveVenueDenormalizedFields(ctx, nextVenue);
      if (venueFields.venueId !== undefined) {
        throw new Error("A known public promotion venue must bind its exact venueId.");
      }
    }
    const timePatch = normalizeEventTimeWritePatch({
      time: nextTime,
    ...
    const currentPendingReasons = Array.isArray(
      ...
      : [];
    const currentSignals = Array.isArray(primaryContext.currentFields.moderationSignals)
    ...
    if (
      ...
      throw new Error("Reviewed promotion folding did not bind the corrected event.");
    }
    await assertPersistedServiceSourcePolicy(ctx, effectivePrimary, {
      allowHumanReviewedStructured: true,
    });
    await assertApprovalCandidatePolicy(ctx, effectivePrimary, [primary._id, variant._id]);
    if (!(await isCanonicallyGroundedApprovedEvent(ctx, effectivePrimary))) {
      throw new Error("Reviewed promotion fold would not remain publicly grounded.");
    }
    ...
    const primaryExpected = bindingForKey(primaryContext.sourceLink.sourceOccurrenceKey);
    const variantExpected = bindingForKey(variantContext.sourceLink.sourceOccurrenceKey);
    if (
      !sourceOccurrenceRepresentativeMatchesExpected(effectivePrimary, primaryExpected) ||
      !sourceOccurrenceRepresentativeMatchesExpected(effectivePrimary, variantExpected)
    ) {
      throw new Error("Reviewed promotion fold would not satisfy both source receipts.");
    }
    ...
    const [variantSavedEvents, variantUserSavedEvents] = await Promise.all([
      ctx.db
        .query("savedEvents")
        .withIndex("by_event", (q) => q.eq("eventId", variant._id))
    ...
    if (
      variantSavedEvents.length > MAX_CROSS_POST_PROMOTION_SAVES_PER_EVENT ||
      variantUserSavedEvents.length > MAX_CROSS_POST_PROMOTION_SAVES_PER_EVENT
    ) {
      throw new Error("Reviewed promotion folding save cohort exceeds the safe bound.");
    }
    const saveResult = await reassignSavedEventReferences(ctx, variant._id, primary._id);
    await ctx.db.patch(primary._id, {
      ...timePatch,
      ...venueFields,
      title: nextTitle,
    ...
    await ctx.db.patch(primaryContext.receipt._id, {
      ...
      satisfiedOccurrences: [
        { key: primaryExpected.key, eventId: primary._id },
      ],
      updatedAt: primaryReceiptUpdatedAt,
    });
    const variantModerationNote =
      `[reviewed_promotion_variant:v1] ${operationId} - ${moderationNote}`;
    ...
    await writeEventAuditLog(ctx, variant._id, "reviewed_promotion_variant_rejected", {
      actor: authorization.actor,
      note: moderationNote,
      patch: variantAuditPatch,
    });
    await writeEventAuditLog(ctx, primary._id, "reviewed_promotion_variant_folded", {
      actor: authorization.actor,
      note: moderationNote,
    ...
    if (
      ...
      throw new Error("Reviewed promotion fold failed its final grounding proof.");
    }
    return {
      operationId,
      primaryId: primary._id,
...
/**
 * Folds a continuation explicitly described as part of the same overnight
 * program into its already-confirmed starting occurrence. A different-day
 * sibling from the same post receives its reviewed poster time/venue correction
 * in the same transaction, and every original receipt key remains present so
 * future ingestion cannot recreate the split row.
 */
export const foldReviewedStructuredSameSourceContinuation = mutation({
  ...
  handler: async (ctx, args) => {
    const authorization = await requireAdminOrServiceSecret(ctx, args.serviceSecret);
    if (authorization.kind !== "service") {
      throw new Error("Reviewed continuation folding requires service authentication.");
    }
    const operationId = args.operationId.trim();
    ...
    const independentPosterTimeEvidence = args.independentPosterTimeEvidence
      .normalize("NFKC")
      .trim();
    const independentPosterArtistEvidence = args.independentPosterArtistEvidence.map(
    ...
    for (const value of [
      ...
      if (!Number.isSafeInteger(value)) {
        throw new Error("Reviewed continuation folding requires safe optimistic revisions.");
      }
    }
    ...
    if (
      ...
      independent.normalizedFieldsJson !== args.expectedIndependentNormalizedFieldsJson ||
    ...
    if (!publiclyGrounded.every(Boolean)) {
      throw new Error("Reviewed continuation folding requires grounded approved events.");
    }
    ...
    const [primaryContext, continuationContext, independentContext] =
      await Promise.all([
        ...
        loadReviewedStructuredCorrectionContext(ctx, continuation, {
          ...
          expectedSourceLinkUpdatedAt: args.expectedContinuationSourceLinkUpdatedAt,
          ...sharedReceiptArgs,
        }),
        loadReviewedStructuredCorrectionContext(ctx, independent, {
          expectedSourceLinkId: args.expectedIndependentSourceLinkId,
          expectedSourceLinkUpdatedAt: args.expectedIndependentSourceLinkUpdatedAt,
          ...sharedReceiptArgs,
        }),
      ]);
    ...
    const exactPostUrl = normalizeInstagramPostUrl(primary.instagramPostUrl);
    if (
      ...
      !exactPostUrl ||
      normalizeInstagramPostUrl(continuation.instagramPostUrl) !== exactPostUrl ||
      normalizeInstagramPostUrl(independent.instagramPostUrl) !== exactPostUrl ||
      ...
      throw new Error("Reviewed continuation folding source topology is not exact.");
    }
    ...
    if (
      ...
      throw new Error("Reviewed continuation folding time evidence is not exact.");
    }
    ...
    if (!Array.isArray(scheduleEntries) || scheduleEntries.length !== 3) {
      throw new Error("Reviewed continuation folding requires the exact three-row schedule.");
    }
    ...
    const primaryScheduleArtists = Array.isArray(primarySchedule?.artists)
      ...
      : [];
    const continuationScheduleArtists = Array.isArray(continuationSchedule?.artists)
    ...
    if (
      ...
      throw new Error("Reviewed continuation folding schedule evidence is not exact.");
    }
    ...
    const deterministicDescription = [primary.description, continuation.description]
    ...
    if (
      ...
      throw new Error("Reviewed continuation folding final lineup is not deterministic.");
    }
    ...
    if (
      ...
      throw new Error("Reviewed continuation fold target venue is not exact and public.");
    }
    const venueFields = resolveVenueDenormalizedFieldsFromPublicVenues(
      [targetVenue],
      nextVenue,
    ...
    if (venueFields.venueId !== targetVenue._id) {
      throw new Error("Reviewed continuation fold target venue did not resolve exactly.");
    }
    ...
    const currentPendingReasons = Array.isArray(primaryFields.moderationPendingReasons)
    ...
    if (
      ...
      throw new Error("Reviewed continuation fold did not bind its final public events.");
    }
    await assertPersistedServiceSourcePolicy(ctx, effectivePrimary, {
      allowHumanReviewedStructured: true,
    });
    ...
    if (
      ...
      throw new Error("Reviewed continuation fold would not remain publicly grounded.");
    }
    ...
    const nextExpectedOccurrences = receipt.expectedOccurrences.map((occurrence) =>
    ...
    const nextSatisfiedOccurrences = receipt.satisfiedOccurrences.map((occurrence) =>
      occurrence.key === primaryKey || occurrence.key === continuationKey
        ? { key: occurrence.key, eventId: primary._id }
        : occurrence,
    );
    if (
      nextExpectedOccurrences.filter((item) => item.key === independentKey).length !== 1 ||
      nextSatisfiedOccurrences.filter(
        (item) => item.key === independentKey && item.eventId === independent._id,
      ).length !== 1
    ) {
      throw new Error("Reviewed continuation fold lost the independent occurrence.");
    }
    for (const satisfied of nextSatisfiedOccurrences) {
      ...
      if (
        ...
        throw new Error("Reviewed continuation fold would invalidate its receipt.");
      }
    }

    const [continuationSavedEvents, continuationUserSavedEvents] =
      await Promise.all([
        ctx.db
          .query("savedEvents")
          .withIndex("by_event", (q) => q.eq("eventId", continuation._id))
          .take(MAX_CROSS_POST_PROMOTION_SAVES_PER_EVENT + 1),
        ctx.db
          .query("userSavedEvents")
          .withIndex("by_event", (q) => q.eq("eventId", continuation._id))
    ...
    if (
      continuationSavedEvents.length > MAX_CROSS_POST_PROMOTION_SAVES_PER_EVENT ||
      continuationUserSavedEvents.length > MAX_CROSS_POST_PROMOTION_SAVES_PER_EVENT
    ) {
      throw new Error("Reviewed continuation fold save cohort exceeds the safe bound.");
    }
    ...
    const independentUpdatedAt = nextEventUpdatedAt(independent.updatedAt, reviewedAt);
    const receiptUpdatedAt = nextEventUpdatedAt(receipt.updatedAt, reviewedAt);
    const continuationModerationNote =
      `[reviewed_same_source_continuation:v1] ${operationId} - ${moderationNote}`;
    const receiptAfter = {
      ...receipt,
      expectedOccurrences: nextExpectedOccurrences,
    ...
    if (
      ...
      throw new Error("Reviewed continuation fold failed its final grounding proof.");
    }
    return {
      operationId,
      primaryId: primary._id,
