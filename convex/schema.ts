...
export default defineSchema({
  ...
  events: defineTable({
    ...
    automaticUniqueApprovalPolicyVersion: v.optional(v.literal(1)),
  ...
  automaticUniqueApprovalState: defineTable({
    key: v.literal("automatic-unique-approval-v1"),
    cursor: v.union(v.string(), v.null()),
    completedCycles: v.number(),
    scannedCount: v.number(),
    approvedCount: v.number(),
    classificationErrorCount: v.optional(v.number()),
    lastCycleCompletedAt: v.optional(v.number()),
    createdAt: v.number(),
    updatedAt: v.number(),
  }).index("by_key", ["key"]),
