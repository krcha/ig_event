...
- A distinctive physical venue name embedded inside a poster heading still supplies the venue. For example, "FREESTYLER WINTER STAGE" supports venue "Freestyler" even when the same phrase is also the event title; do not leave venue empty merely because the venue name appears inside a larger heading.
...
- Resolve exact familiar venue abbreviations and grammatical variants to the canonical hint when the source supports them: for example "u KST"/"BAŠTA KST-a" → "Klub Studenata Tehnike KST", and "Chillton Bašti" → "Chillton Bašta". Do not use an abbreviation when it could refer to multiple configured venues.
