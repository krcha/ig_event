...
- Normalize grammatical case when the physical venue identity is unambiguous. For example, a Serbian locative ending such as "... Stazi" can identify a configured canonical "... Staza"; it is venue evidence, not an event title.
- A distinctive configured physical venue name embedded inside a larger poster heading still supplies the venue. A heading shaped like "<VENUE NAME> WINTER STAGE" can support that venue even when the whole phrase is also the event title; do not leave venue empty merely because the venue name appears inside a larger heading.
...
- Preserve an explicitly billed Instagram artist handle exactly with its leading @. When a poster display name and one caption handle clearly identify the same billed act, keep the handle once instead of returning both spellings, and do not report that harmless alias as a conflict.
...
- For exactly ONE non-recurring real event occurrence, merge complementary evidence across the poster and caption: for example, the poster may supply the date/time/venue while the caption explicitly bills or affirmatively announces the title or artist. A music/performance clause such as "O Bato i predivna muzika Brazila još jednom kod nas" is affirmative event-announcement evidence; narrative listening, influence, inspiration, credits, and hashtag-only mentions are not. Put the merged values in the sole "schedule_entries" object, keep "source_text" as an exact source snippet rather than a synthetic quote, and put the exact caption identity snippet in both the title/artists field_confirmation entries that it supports. Never use this rule for a recurring schedule or to carry an identity across two or more schedule rows.
...
- Resolve exact venue abbreviations and Serbian grammatical variants to a canonical hint only when the configured resolver makes the identity unambiguous (including genitive abbreviation suffixes such as "-a" and locative "Bašti" → canonical "Bašta"). Do not use an abbreviation when it could refer to multiple configured venues.
- Prefer the explicitly named physical location over the source account, promoter, organizer, event-brand, sponsor, ticket seller, or ticket-domain name. A poster that names a physical venue outranks an unrelated event-brand account or ticket URL. Use a canonical venue hint from a venue account only when no different physical venue is explicitly named. Preserve a billed artist's Instagram handle when the handle is the clearest source identity.
...
- When "schedule_entries" is populated, leave top-level "date", "time", "title", "artists", and "description" empty or [] unless there is also one single poster-wide value that clearly applies to every entry. When there is exactly one real occurrence, mirror its final merged values into both the top level and the sole schedule entry.
...
- Compare claims for the same occurrence only. A performer or account named on Saturday is not a conflicting artist for a separately dated Sunday row. A film's general theatrical release date (for example "U BIOSKOPIMA 10. SEPTEMBAR") is not a screening date that conflicts with a venue program running on later dates; use each row's screening date from that program.
