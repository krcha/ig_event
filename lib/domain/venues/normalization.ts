/** Authoritative, provider-agnostic venue normalization and evidence policy. */
...
export function getConfiguredVenueNameForHandle(
  ...
  return (
    canonicalVenueNamesByHandle[normalized] ??
    staticVenueByHandle[normalized] ??
    ""
  );
}
...
function buildCanonicalVenueEntries(
  ...
  for (const [handle, aliases] of Object.entries(
    canonicalVenueAliasesByHandle,
  )) {
...
function findEntryByVenueName(
  ...
  return findUniqueVenueEntry(
    entries.filter(
      (entry) => normalizeVenueComparableText(entry.name) === normalizedName,
    ),
  );
}
...
export function canonicalizeVenueNameDetailed(
  ...
  const canonicalVenueAliasesByHandle =
    options?.canonicalVenueAliasesByHandle ?? {};
  ...
  if (preferredVenue && areVenueNamesCompatible(candidate, preferredVenue)) {
    const preferredEntry = findEntryByVenueName(
      preferredVenue,
      canonicalVenueEntries,
    ) ?? {
      name: preferredVenue,
      handle: null,
    };
  ...
  const candidateTokenCount = normalizedCandidate
    .split(" ")
  ...
  const compatibleMatch = findUniqueVenueEntry(
    canonicalVenueEntries.filter(
      (entry) =>
        !entry.matchedAlias && areVenueNamesCompatible(candidate, entry.name),
    ),
  );
...
function pickExplicitVenueCandidate(
  ...
  const hasModelVenue =
    modelVenue.length > 0 && !isLowConfidenceVenue(modelVenue);
...
function hasImmediateLocativeHandleContext(
  ...
  const clause = getImmediateEvidenceClause(
    evidence.slice(contextStart, mentionStart),
  );
  ...
  const directCueMatch =
    /\b(?:at|in|u|na|kod|venue|location|lokacija|mesto)\s*(?::|-)?\s*$/iu.exec(
      clause,
    );
  if (directCueMatch) {
  ...
  if (
    !vidimoSeMatch ||
    hasRecentLocativeNegation(clause.slice(0, vidimoSeMatch.index))
  ) {
    return false;
  }
...
function hasShortVenueNameContext(
  ...
  const nearbyPrefix = evidenceTokens.slice(
    Math.max(0, startToken - 2),
    startToken,
  );
...
function findUniqueCanonicalVenueNameMention(
  ...
  for (const [evidenceIndex, rawEvidence] of evidenceTexts.entries()) {
    ...
    const evidenceTokens = toSearchableText(evidence)
      .split(" ")
      .filter(Boolean);
    ...
    for (const entry of entries) {
      ...
      for (const startToken of findExactTokenPhraseStarts(
        ...
        if (
          !hasShortVenueNameContext(evidenceTokens, startToken, venueTokens)
        ) {
          continue;
        }
        matches.push({
  ...
  // compound venue name win over a separately configured shorter alias, while
  // a second standalone occurrence of the short alias still makes the
  // evidence ambiguous and therefore fails closed.
  const unshadowedMatches = matches.filter(
    (match) =>
      !matches.some(
        (other) =>
...
export function normalizeVenueFromEvidence(
  ...
  const canonicalVenueAliasesByHandle =
    input.canonicalVenueAliasesByHandle ?? {};
  const allowCanonicalHandleFallback =
    input.allowCanonicalHandleFallback !== false;
  ...
  const mappedVenue = allowCanonicalHandleFallback
    ...
    : "";
  const locationName = trimWrappedPunctuation(
    normalizeString(input.locationName),
  );
  const modelVenue = trimWrappedPunctuation(
    normalizeString(input.rawModelVenue),
  );
...
export function normalizeExtractedArtists(values: string[]): string[] {
  ...
  for (const value of values) {
    const cleaned = trimWrappedPunctuation(normalizeString(value)).replace(
      /\s+/g,
      " ",
    );
    if (!cleaned) {
    ...
    if (
      !comparable ||
      GENERIC_ARTIST_VALUES.has(comparable) ||
      seenArtists.has(comparable)
    ) {
      continue;
    }
